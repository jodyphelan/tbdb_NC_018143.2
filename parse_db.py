...
def parse_mutation(mut,gene,fasta_dict,gene_info):
	...
	if re_obj:
		...
		seq = fasta_dict["NC_018143.2"][chr_start_nt-2:chr_start_nt]
	...
	if re_obj:
		...
		seq = fasta_dict["NC_018143.2"][chr_start_nt-2:chr_end_nt]
		return ["%s%s>%s" % (chr_start_nt-1,seq,seq[0])]
	# Insertion
	...
	if re_obj:
		...
		seq_start = fasta_dict["NC_018143.2"][chr_start_nt-1]
...
def main(args):
	...
	open(genome_file,"w").write(">%s\n%s\n" % (chr_name,fasta_dict["NC_018143.2"]))
	subprocess.call("sed 's/NC_018143.2/%s/g' genome.gff > %s" % (chr_name,gff_file),shell=True)
	subprocess.call("sed 's/NC_018143.2/%s/g' barcode.bed > %s" % (chr_name,barcode_file),shell=True)
...
parser.add_argument('--seqname','-s',default="NC_018143.2",type=str,help='The prefix for all output files')
